...
Now, let's get the enriched metabolic pathways. First compute clusters.
```{r}
...
Now let's the differentially expressed metabolic pathways.
```{r}
...
Plot barplots of Reactome pathway enrichments for each cluster.
```{r}
plot_gsva_pathway(gsva_result,
                  pathway_id=rownames(max_difference)[2])
```

Plot clustergram of Reactome annotations.
```{r}
plot_gsva_heatmap(gsva_result, 
                  max_pathways=15, 
                  margins=c(6,20)) #+
  #labs(x="Clusters", 
  #     y="Reactome Annotations")
```
