...
Let's also create a function that grabs the metabolic genes from the data matrix.
```{r}
...
Let's clean up the count matrix.
```{r}
...
orig.ident = lung_metabolism@active.ident
save(lung_metabolism, file="C:/Users/scott/Data/scRNASeq/shea/lung_human.robj")
...
```{r}
#lung = PercentageFeatureSet(lung_metabolism, 
#                            pattern = "^mt-", 
#                            col.name = "percent.mt")
#lung = SCTransform(lung_metabolism,
...
top10 = head(VariableFeatures(lung_metabolism), 10)
...
```{r}
lung_metabolism@active.ident=orig.ident
DimPlot(lung_metabolism, 
        reduction="umap",
        dims=c(1, 2),
        label=TRUE,
        repel=TRUE,
        ...
        na.value = "grey50") +
  NoLegend() +
  labs(x="UMAP 1", y="UMAP 2", title="Lung metabolism UMAP embedding")
```
```{r}
FeaturePlot(lung_metabolism, 
            features="Ldha",
            label=TRUE,
            repel=TRUE,
            label.size=4)
...
Find markers
```{r}
markers = FindAllMarkers(lung_metabolism,
                         only.pos=TRUE,
                         min.pct = 0.25,
...
## Neutrophil-specific markers against all cell types.
```{r}
neutrophil_markers = markers[markers$cluster=='Neutrophils',]
FeaturePlot(lung_metabolism, 
            features=as.character(neutrophil_markers$gene[1:10]),
            label=TRUE,
            repel=TRUE,
            label.size=4)

tmp = neutrophil_markers$gene
```
