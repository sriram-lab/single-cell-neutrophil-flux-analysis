...
save(neutrophils, file='D:/Data/scRNASeq/shea/neut_SEC.robj')
```
...
markers = markers[markers$pct.1 > 0.5 & markers$pct.2 < 0.5, ]
#write_csv(markers, "D:/Analysis/Immune/neutrophil_deg_clusters.csv")
```
```{r, fig.width=12}
...
neutrophils = SetIdent(obj=neutrophils, 
                       value=neutrophils@meta.data$Time)
```
...
DimPlot(neutrophils, 
        ...
        label.size=4,
        na.value = "grey50") + 
        labs(x="UMAP 1", y="UMAP 2", title="Time-Course") +
        theme_minimal() +
        theme(plot.title = element_text(size=25),
              legend.text=element_text(size=20),
              axis.title.x=element_text(size=20),
...
markers = markers[!duplicated(markers[, "gene"]), ]
markers = markers[markers$pct.1 > 0.5 & markers$pct.2 < 0.5, ]
write_csv(markers, "D:/Analysis/Immune/neutrophil_deg_timecourse.csv")
```
